...
if [ "$cygwin" = "false" ] && [ "$darwin" = "false" ] && [ "$nonstop" = "false" ] ; then
    ...
    if [ $? -eq 0 ] ; then
        ...
        warn "Could not query maximum file descriptor limit ($MAX_FD_LIMIT), continuing with the default"
    fi
fi

# For Darwin, add paths to locate shared libraries
if $darwin ; then
    if [ -z "$JAVA_OPTIONS" ] ; then
        JAVA_OPTIONS="-Djava.library.path=/usr/lib:/usr/local/lib"
    fi
fi
